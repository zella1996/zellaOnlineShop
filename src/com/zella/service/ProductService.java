...
import com.zella.dao.ProductDao;
import com.zella.domain.Category;
import com.zella.domain.Order;
import com.zella.domain.PageBean;
...
public class ProductService {
	...
	public boolean submitOrder(Order order) {
		...
		try {
			...
			// 2.调用dao层存储order表数据的方法
			dao.addOrder(order);
			// 3.调用dao层存储orderItem表数据的方法
	...
	public void updateReceiverInfo(Order order) {

		ProductDao dao = new ProductDao();
		try {
			dao.updateReceiverInfo(order);
		} catch (SQLException e) {
			e.printStackTrace();
		}
